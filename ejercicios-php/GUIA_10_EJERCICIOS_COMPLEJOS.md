...
Juego en una sola página: al cargar por primera vez (sin parámetros), el script genera un número aleatorio del 1 al 100 y lo guarda en **sesión**… **No:** aún no vieron sesiones de forma obligatoria. Entonces: versión sin sesión — el número secreto se pasa de un envío a otro usando un **campo oculto** `input type="hidden"` en un formulario POST con el valor del número (o usar GET con cuidado de no “spoilear” en la barra si prefieren POST).
...
## Criterios de evaluación sugeridos (para el docente)

- **Correctitud:** el programa hace lo pedido en casos normales y no rompe en casos límite razonables.
- **Estructura:** funciones donde aporta claridad; poco código duplicado.
- **Consistencia:** mismo estilo de nombres y formato; comentarios solo donde aportan.
- **Seguridad básica:** escape al imprimir datos de usuario; validación server-side en formularios.

---

*Documento generado para Programación II — Nueva Escuela de Diseño y Comunicación. Los enunciados pueden adaptarse (nombres de película, moneda, cantidad de ítems) sin cambiar la dificultad.*
